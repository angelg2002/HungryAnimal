...
function maquetadordeproductosC() {
    const contenedorCD = document.getElementById('contenedor-domesticos');
    const filtroPD = articulos.filter(articulo => articulo.categoria === 'domestico');
    if (!contenedorCD) return;

    if (filtroPD.length === 0) {
        contenedorCD.innerHTML = `<p>no hay productos domestiocos para mostrar</p>`;
        return;
    }

    let htmlString = ''

    filtroPD.forEach(articulo => {
         htmlString += `
...
//para activar el botón se tiene que leer la función de botón dentro del maquetador HTML 
