console.log('el maquetador de perros se está leyendo');

// Variable global para guardar los productos de perros
let articulosPerros = [];

// 1. CONEXIÓN: Carga los productos con fallback a JSON local
async function cargarProductosPerros() {
  try {
    const respuesta = await fetch('http://localhost:3000/api/productos');
    if (!respuesta.ok) throw new Error('Backend no disponible');
    const todosLosProductos = await respuesta.json();
    articulosPerros = todosLosProductos.filter(articulo => articulo.especie === 'Perro');
  } catch (error) {
  ...
  maquetadordeproductosP(articulosPerros);
}

// 2. MAQUETACIÓN (Ahora recibe los artículos por parámetro)
function maquetadordeproductosP(articulos) {
  const contenedorPerros = document.getElementById('contenedor-perros');
  if (!contenedorPerros || !articulos) return;

  if (articulos.length === 0) {
    contenedorPerros.innerHTML = `<p>No hay productos para perros para mostrar</p>`;
    return;
  }
  ...
  articulos.forEach(articulo => {
    const botonesOpciones = articulo.presentaciones.map((pres, index) => `
            <button onclick="seleccionarOpcionP(${articulo.id}, ${pres.precio}, '${pres.nombre}', this)" 
  ...
  contenedorPerros.innerHTML = htmlString;
  activarBotonesCarrito();
}

// 3. SELECCIÓN DE OPCIÓN
window.seleccionarOpcionP = (idProducto, precioNuevo, nombrePres, botonTocado) => {
...
function activarBotonesCarrito() {
  ...
  botones.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const articuloOriginal = articulosPerros.find(a => a.id === id);
...
document.addEventListener('DOMContentLoaded', cargarProductosPerros);
