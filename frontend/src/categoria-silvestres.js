console.log('el maquetador de silvestres se está leyendo');

// Variable global para guardar los productos silvestres
let articulosSilvestres = [];
...
async function cargarProductosSilvestres() {
    try {
        ...
        articulosSilvestres = todosLosProductos.filter(articulo => articulo.categoria === 'silvestre');

        maquetadordeproductosC(articulosSilvestres);
    } catch (error) {
        console.error("Error al cargar productos silvestres:", error);
    }
}

// 2. MAQUETACIÓN (Ahora recibe los artículos por parámetro)
function maquetadordeproductosC(articulos) {
    const contenedorCD = document.getElementById('contenedor-silvestres');
    if (!contenedorCD || !articulos) return;

    if (articulos.length === 0) {
        contenedorCD.innerHTML = `<p>no hay productos silvestres para mostrar</p>`;
        return;
    }
...
function activarBotonesCarrito() {
    ...
    botones.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const articuloOriginal = articulosSilvestres.find(a => a.id === id);
...
document.addEventListener('DOMContentLoaded', cargarProductosSilvestres);